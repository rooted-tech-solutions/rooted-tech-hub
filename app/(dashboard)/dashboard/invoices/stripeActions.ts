...
export async function createStripeCheckout(invoiceId: string) {
  ...
  if (!invoice || !invoice.amount) {
    throw new Error("Invoice not found or has no amount");
  }
  ...
  const clientName =
    ...
    (invoice.clients as { company?: string | null; name?: string | null } | null)?.name ||
    "Client";
  const clientEmail =
    (invoice.clients as { email?: string | null } | null)?.email ?? undefined;
    ...
    line_items: [
      {
        price_data: {
          ...
          product_data: {
            name: `${invoice.title} — ${typeLabel}`,
            description: `Invoice ${invoice.invoice_number} · ${clientName}`,
          },
        },
        quantity: 1,
      },
